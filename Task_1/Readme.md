...
# Замечание по ревью:
Kafka и Integration Service не заменят ESB один к одному:  
стоит явно показать, куда переедут ФЛК, трансформации (потоковые и пакетные),  
маршрутизация, ретраи и интеграционная/бизнес логика.

# Исправления и объяснения

Спасибо за замечание.  
Когда я читал задание и перечень используемых технологий/сервисов:
* DWH на базе Microsoft SQL-сервера 2008 года;
* Power BI;
* Power Builder;
* ESB на базе Apache Camel;

То по собственному незнанию посмотрев на устаревшие `Power Builder` и `Microsoft SQL-сервера 2008` отнёс к устаревшим и `Power BI` и `ESB на базе Apache Camel`.  
Теперь я познакомился поближе с Apache Camel, его применении в построении ESB. Узнал новый термин - EIP и то, как много этих паттернов реализовано в Apache Camel.

Вердикт такой, что контейнеры с `Power BI` и `ESB на базе Apache Camel` предлагаю оставить в будущей архитектуре.  
Для `Power BI` предлагается добавить интеграцию с новым блоком контейнеров Data Lakehouse.  
На `ESB/Camel` выпадает задача интеграции новых компонентов - маршрутизация, трансформация. При этом поддержка действующего интеграционного слоя также остаётся на этом компоненте.

Однако, т.к. компания «Будущее 2.0» планирует перенести основную часть своего IT-ландшафта в облачную инфраструктуру, то с `Power BI`
могут возникнуть сложности - привязка к `Microsoft Azure`, что сейчас неприемлемо из-за санкций. Предлагаю переход на `Yandex DataLens`. Заодно будет решена проблема вендор-лок от Microsoft.


Если по каким-то причинам решение оставить Apache Camel неприемлемо, то готов заменить его на инеграционный контейнер и контейнер для событий.  
Это будет во второй схеме.

## Схема архитектуры

Диаграмма С4 с Apache Camel: ![task1_c4.drawio](task1_c4.drawio.png)

Исходный код диаграммы: [task1_c4.drawio](task1_c4.drawio).

Диаграмма С4 c новым интеграционным слоем: ![task1_c4_event.drawio](task1_c4_event.drawio.png)

Исходный код диаграммы: [task1_c4_event.drawio](task1_c4_event.drawio).
